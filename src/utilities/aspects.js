...
export const getAspectData = (horoscope) => {
	...
	return {...Object.assign({}, ...data.map ((aspect) => ({ [aspect.key]: aspect })))};
};
			...
			aspectWithPoints
				...
				.forEach ((point2) => {
					...
					if (isAspect (point1Data.point, point2Data.point, aspectObject.angle, maxOrb)) {
						...
						isAspect (point2Data.point, point1Data.point, aspectObject.angle, maxOrb)) {
						orb = calculateOrb(aspectObject.angle, maxOrb, point1Data.point, point2Data.point);
						aspect = new Aspect ({
							aspectKey: type,
							point1Key: point1,
					...
					if (!aspect) return;
					if (aspect) {
						if (aspect.point1Key === "northnode" && aspect.point2Key === "southnode") {
							return; // no northnode-southnode aspect
						}
						if (aspect.point1Key === "southnode" && aspect.point2Key === "northnode") {
							return; // no southnode-northnode aspect
						}
						if (aspect.point1Key === "ascendant" && aspect.point2Key === "midheaven") {
							return; // no ascendant-midheaven aspect
						}
						if (aspect.point1Key === "midheaven" && aspect.point2Key === "ascendant") {
							return; // no midheaven-ascendant aspect
						}
						if (aspects.find ((a) => a.point1Key === point2 && a.point2Key === point1)) {
